...
Extra dependency groups for notebooks and development (`pip install -e ".[notebooks]"`, `pip install -e ".[dev,train]"`) are described in the [upstream README](https://github.com/facebookresearch/sam3#installation).
...
If you use SAM 3 (or this tool) in your research, please cite:
...
This repository is derived from Meta's [SAM 3](https://github.com/facebookresearch/sam3) and is released under the same [SAM License](LICENSE); the `app/` annotation tool is published under the same terms. The original upstream documentation and contribution guidelines remain available at the [upstream repository](https://github.com/facebookresearch/sam3) and in [CONTRIBUTING.md](CONTRIBUTING.md).
